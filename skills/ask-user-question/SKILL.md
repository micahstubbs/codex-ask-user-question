...
In Codex, use the native `request_user_input` tool by default whenever it appears in the session's
available tools. This includes Default mode when `features.default_mode_request_user_input` is
enabled. Do not infer that the tool is unavailable merely because the session is in Default mode.
Use the Markdown fallback only when the tool is absent or the runtime rejects the call as
unavailable.
...
- Call `request_user_input` whenever it appears in the available tools; do not voluntarily replace
  it with a prose or Markdown question.
- If the tool is absent, or a call is rejected because the runtime forbids it, use the Markdown
  fallback and state the limitation once.
...
### Native-first availability rule

Treat `request_user_input` as available when it is listed in the current session's tools, regardless
of collaboration-mode label. On Codex versions that expose the Default-mode feature gate, enable it
once with:
...
The setting is loaded when a Codex process starts, so restart or reopen the session after enabling
it. Skills do not register native tools themselves; they use the tools Codex provides.
...
If `request_user_input` is absent or the runtime rejects the call as unavailable, ask one concise
Markdown question:
...
- Do not choose Markdown merely because the collaboration mode is Default.
